...
const JD_TEXT = 'We are hiring a senior backend engineer with NestJS experience.'
const CV_TEXT = 'Senior backend engineer, 6 years experience with Node.js and NestJS.'
...
async function advanceToReview(page: Page): Promise<void> {
  ...
  // tests exercise.
  await expect(page.getByRole('textbox', { name: 'Job Description' })).toHaveValue(JD_TEXT)
  await expect(page.getByRole('textbox', { name: 'CV / Resume' })).toHaveValue(CV_TEXT)
}
...
test.describe('step 3 -> 4 — Run match (route-stubbed)', () => {
  test('Run match advances to step 4 and renders the stubbed result report', async ({ page }) => {
    ...
    // Step 4 (Result).
    await expect(page.getByRole('heading', { name: 'Review Parsed Data' })).toBeHidden()
    ...
    await expect(page.getByText(`${STUB_MATCH_RESULT.overallScore}%`)).toBeVisible()
    await expect(page.getByText('Overall match')).toBeVisible()
    await expect(page.getByText('Semantic match')).toBeVisible()
    await expect(page.getByText(`${STUB_MATCH_RESULT.semanticScore}%`)).toBeVisible()
    await expect(page.getByText('Keyword / Skills match')).toBeVisible()
    await expect(page.getByText(`${STUB_MATCH_RESULT.keywordScore}%`)).toBeVisible()
  ...
  test('i18n: step-4 result labels render translated VI copy via the dev __i18n hook', async ({
    ...
    await page.evaluate(
      () =>
        (window as unknown as { __i18n: { changeLanguage: (l: string) => void } }).__i18n
          .changeLanguage('vi'),
    )
  ...
  test('Start over on step 4 resets the wizard back to step 1', async ({ page }) => {
    await stubMatchApi(page)
    await advanceToReview(page)
    await page.getByRole('button', { name: 'Run match' }).click()
    await expect(page.getByText('Overall match')).toBeVisible()
