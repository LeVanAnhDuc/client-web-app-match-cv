...
test.describe('file validation', () => {
  test('[EP] wrong file type (.exe) is rejected client-side', async ({ page }) => {
  ...
  test('[EP] wrong file type (.png) is rejected client-side', async ({ page }) => {
    await gotoWizard(page)
  ...
  test('[BVA] file over the 10MB limit is rejected (valid extension)', async ({ page }) => {
    await gotoWizard(page)
  ...
  test('[DT] wrong-type + oversize: type error takes precedence over size', async ({ page }) => {
    await gotoWizard(page)
