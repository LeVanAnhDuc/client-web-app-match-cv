...
async function changeLanguage(page: Page, lng: 'en' | 'vi'): Promise<void> {
  await page.evaluate(
    (l) => (window as unknown as { __i18n: { changeLanguage: (l: string) => void } }).__i18n
      .changeLanguage(l),
    lng,
  )
}
...
test.describe('i18n', () => {
  ...
  test('switching to VI renders VI copy for stepper, step title, and empty state', async ({
    ...
    await expect(page.getByRole('heading', { name: 'Nhập mô tả công việc' })).toBeVisible()
  ...
  test('switching back to EN restores EN copy (round-trip, no stuck state)', async ({ page }) => {
    await gotoWizard(page)
    await changeLanguage(page, 'vi')
    await expect(page.getByText('Bước 1/4')).toBeVisible()
