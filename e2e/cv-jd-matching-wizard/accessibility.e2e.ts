...
test.describe('accessibility', () => {
  test('Next/Back are real buttons with accessible names', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Back' })).toHaveRole('button')
  })

  test('active stepper step exposes aria-current="step"', async ({ page }) => {
    await gotoWizard(page)
    await expect(page.getByTestId('stepper-step-1')).toHaveAttribute('aria-current', 'step')
    await expect(page.getByTestId('stepper-step-2')).not.toHaveAttribute('aria-current', 'step')
  })

  test('saved-doc reuse list exposes a radiogroup with labeled options', async ({ page }) => {
    ...
    await expect(page.getByRole('radio', { name: new RegExp(title) })).toBeVisible()
  })
