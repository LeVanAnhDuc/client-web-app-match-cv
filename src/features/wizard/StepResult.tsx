...
import { AlertTriangle, CircleCheck, Lightbulb, Loader2, RotateCcw } from 'lucide-react'
import { useTranslation } from 'react-i18next'
import { useMatchResult } from '#/features/matching/queries'
    ...
    <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-2xl border border-slate-100 dark:border-slate-700/50 overflow-hidden">
      <div className="p-10 flex flex-col md:flex-row items-center gap-12 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-900/30">
        ...
        <div className="flex-1 w-full space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <ScoreBar label={t('result.semantic')} value={data.semanticScore} />
      ...
      <div className="p-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
        <ReportList
          ...
          itemIcon={<CircleCheck className="text-green-500 shrink-0 mt-0.5" size={18} />}
        />
        <ReportList
          icon={<AlertTriangle className="text-amber-600 dark:text-amber-500" size={18} />}
          title={t('result.gaps')}
          items={data.report.gaps}
          itemIcon={<AlertTriangle className="text-amber-500 shrink-0 mt-0.5" size={18} />}
        />
      </div>
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-sm font-bold text-blue-600 dark:text-indigo-400">{value}%</span>
      </div>
