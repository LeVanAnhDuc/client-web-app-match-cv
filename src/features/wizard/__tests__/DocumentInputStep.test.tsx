...
import type { DocumentSummaryDto } from '#/features/documents/types'
...
function stubSavedDocs(docs: Array<DocumentSummaryDto>) {
  const fetchMock = vi.fn(async () => ({ ok: true, status: 200, json: async () => docs }) as Response)
...
describe('DocumentInputStep', () => {
  it('renders the Upload file / Paste text tabs', async () => {
    stubSavedDocs([])
    render(<DocumentInputStep kind="JD" onNext={vi.fn()} />, { wrapper: Wrapper })
  ...
  it('switches from Upload to Paste tab', async () => {
    ...
    expect(await screen.findByPlaceholderText(/paste the text content here/i)).toBeDefined()
  })

  it('shows the reuse empty-state when there are no saved documents', async () => {
    stubSavedDocs([])
    render(<DocumentInputStep kind="CV" onNext={vi.fn()} />, { wrapper: Wrapper })
  ...
  it('calls onNext with the saved document id, skipping the create request', async () => {
    ...
    const onNext = vi.fn()
    render(<DocumentInputStep kind="JD" onNext={onNext} />, { wrapper: Wrapper })
  ...
  it('enables Back when onBack is provided (step 2)', async () => {
    ...
    expect(await screen.findByRole('button', { name: /back/i })).not.toBeDisabled()
  })
})
