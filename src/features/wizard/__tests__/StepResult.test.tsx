...
const matchResult: MatchResultDto = {
  ...
  report: {
    ...
    suggestions: ['Quantify design impact with metrics like "Increased conversion by 15%."'],
  },
...
describe('StepResult', () => {
  beforeEach(() => {
    useWizardStore.setState({ step: 4, jdDocId: 'jd-1', cvDocId: 'cv-1', matchId: 'match-1' })
  })
  ...
  it('renders overall/semantic/keyword scores and the three report lists', () => {
    ...
    expect(screen.getByText('Accessibility Standards (WCAG)')).toBeInTheDocument()
    expect(
      screen.getByText('Quantify design impact with metrics like "Increased conversion by 15%."'),
    ).toBeInTheDocument()
  })
